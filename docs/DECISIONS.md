...
- Open items / next:
```
## [2026-06-18] — Claude Code (Opus 4.8) — Team access & server-enforced roles, settlement thank-you, untracked verified
- P1 — Multi-user team with SERVER-ENFORCED permissions (the security-critical core):
  - Data: `business_members(role owner|co_owner|staff, phone, user_id NULL until first login, linked_staff_id, status)` + `performed_by_member_id` on bills/payments/khata + `created_by_member_id` on customers/khata (migration 006 + schema.sql + startup migrator; applied to LIVE `lvymzqwgtykhuhorfcwm`). RLS tenant policy on the new table.
  - Auth: `get_current_business` resolves owner OR member-by-phone, links user_id + flips invited→active on first login, materialises an owner member row, and carries `role`/`member_id`/`linked_staff_id`. `/auth/verify` returns `role` + `needs_onboarding` (invited members skip onboarding; the app lands them on home because `/business` now resolves them).
  - `permissions.py`: `require_owner` / `require_manager` deps (403 for staff). Enforced: analytics → 403 for staff; `/business` PATCH + payment-method writes → manager-only; staff CRUD/attendance/advance/salary → manager-only; customers + khata scoped to `created_by_member_id` for staff; home feed + `get_bill` scoped to `performed_by_member_id` for staff; `/staff` list+detail = self only; inventory zeroes cost + total stock value for staff. Untracked custom-item billing still works for staff. `/team` (manager) CRUD members with owner-vs-co-owner rules; staff member auto-creates a linked staff record. `/team/activity` (manager) = who-billed/who-collected audit; bill read returns `billed_by_name`.
  - Frontend: `Business.role`; BottomNav hides Analytics for staff; Stock hides total-value card + is view-only for staff; invoice shows "Billed by <member>" (managers); Team sheet under Settings (role badges, status chips, add/remove with owner/co-owner rules); settle/thank-you below.
  - VERIFIED LIVE end-to-end (owner +919876543210 adds co-owner + staff): staff role resolves; analytics/`/team`/business-PATCH all 403; staff sees only their own customer (not the owner's), only their own bill in activity, cost=0, 1 staff record (self); co-owner gets analytics 200 but adding a co-owner is 403; owner `/team/activity` shows "Billed by Ravi Staff / Tester".
- P2 — Settlement thank-you: Khata "Settle Up" row gains a WhatsApp glyph (#25D366) → one tap settles in full AND opens wa.me with a warm shop-signed thank-you; the Settle sheet shows a non-blocking "Send thank-you on WhatsApp" after settling. Partial vs full wording; saved/unsaved/typed numbers. No money-logic change.
- P3 — Untracked custom items: confirmed `inventory_items.tracked` is live; re-verified end-to-end (create untracked → bill qty 4, no oversell, grand ₹360 → catalogue row qty 0/tracked false). It was a delivery gap before; now in the shipped 1.0.0 bundle.
- Verify: `tsc` clean; `expo-doctor` 18/18; backend imports clean (55 routes); pytest needs local PG :5544 (not available here) so behavior was verified LIVE instead.
- Ship: backend pushed (Render auto-redeploy, startup migrator no-op since 006 pre-applied to live); `eas update --branch preview` group `12c4b184-b876-4e11-84ad-7833eb250e8b` (runtime 1.0.0, Android+iOS).
- Compatibility: existing beta users are all owners → role 'owner' → full access, behavior identical to before (no hindrance during/after rollout). No reinstall needed for anyone already on the 1.0.0 APK / current iOS Expo Go — they get this via OTA on next open. Only pre-1.0.0 (old appVersion-runtime) installs still need the one-time 1.0.0 APK reinstall from the prior task.
- Owner action: none required. (Test members "Priya Co"/"Ravi Staff" were added on the beta test account +919876543210 during live verification — removable via the new Team sheet.)

## [2026-06-17] — Claude Code (Opus 4.8) — Untracked items, stable runtimeVersion, OTA fragmentation fix
- What changed:
  - FIX 0 (OTA fragmentation): pinned `runtimeVersion` to a fixed `"1.0.0"` in app.json (was policy `"appVersion"`), so every build on this runtime receives every `eas update` and version bumps never split the channel again. `updates.url` + `extra.eas.projectId` already match the org project `68553041-...`; eas.json preview profile keeps channel `preview`, appVersionSource local. Fresh APK built on runtime 1.0.0 (EAS build `22776d56-beb2-4dce-b534-5072552ce109`). **Pre-migration installs (runtime 0.1.0) must reinstall this one new APK once; afterwards all OTA updates reach them. iOS Expo Go testers are unaffected.**
  - FIX 1 (untracked catalogue items): `inventory_items.tracked boolean NOT NULL DEFAULT true` (migration 005 + schema.sql). A custom line typed during billing now saves with `tracked=false` (name + price, no stock). Untracked items: always sellable (skipped by oversell guard, no stock decrement, no StockLedger row in confirm_bill), never flagged low; shown in Stock as "Custom item · no stock" and in bill search with one-tap add. Tapping a Stock item opens the editor (AddInventorySheet reused in edit mode); entering a quantity PATCHes qty and the server flips it to tracked. New `app/migrate.py` runs additive idempotent migrations on startup (lifespan) so the live DB self-heals on deploy — necessary because the Supabase MCP migration path is no longer available. billing_rules.md §2 updated; backend test added. Verified live end-to-end against Render: untracked create → bill qty 3 (no block) → PATCH qty=5 promotes to tracked.
  - FIX 2 (WhatsApp PDF link): bucket `invoices` exists (public, application/pdf, 10 MB) and storage is enabled (URL auto-derived from DATABASE_URL). Live share-link still returns `{"url":null,"reason":"bad_service_key"}` → the `SUPABASE_SERVICE_ROLE_KEY` on Render is invalid for the project (signature verification fails). **Owner must paste the correct legacy `service_role` secret (see below).** No code change needed beyond last session's diagnostics; "Share / Save" already attaches the real PDF as the secondary path.
  - **CORRECTION (project ref):** the LIVE backend (Render `DATABASE_URL`) and local `.env` both use Supabase project **`lvymzqwgtykhuhorfcwm`** — confirmed: the live-created test business `de77d7a6` and the `tracked` column are in that DB. The earlier memory/notes citing `dmcuwrtjtnigktqxccln` were WRONG, which is why FIX 2 kept failing: last session the `invoices` bucket was created and the `service_role` key copied from `dmcuwrtjtnigktqxccln`, so the key failed signature verification against the real project. Bucket `invoices` (public, pdf, 10 MB) has now been created in **`lvymzqwgtykhuhorfcwm`** via direct asyncpg. Owner must paste the `service_role` secret from **project `lvymzqwgtykhuhorfcwm`** → Settings → API.
- Verify: tsc clean; expo-doctor 18/18; backend syntax-checked + new untracked test added (pytest needs local PG :5544, not runnable in this env — verified the behavior live instead).
- Ship: backend pushed (commit `30bf7d4`, Render auto-redeployed, startup migration added the column); `eas update --branch preview` group `8bdcbd03-53ab-42f4-9103-b3a3c465f773` (runtime 1.0.0); APK build `22776d56`.
- OWNER ACTIONS:
  1. Render env `SUPABASE_SERVICE_ROLE_KEY` = the **legacy `service_role` secret** (a long `eyJ…` JWT) from Supabase project `dmcuwrtjtnigktqxccln` → Settings → API → "Legacy anon, service_role API keys" → `service_role` → Reveal → Copy. NOT the anon key, NOT the JWT Secret, no spaces/quotes. (`INVOICE_BUCKET` optional, defaults to `invoices`; `SUPABASE_URL` not needed — auto-derived.)
  2. Re-share the new APK (build `22776d56`); pre-migration testers reinstall once.

## [2026-06-17] — Claude Code (Opus 4.8) — Invoice fixes: discount display, PDF fonts, two-column power wheel, PDF-host diagnosis
- What changed:
  - Discount display (was: discount silently spread across item rows so rate×qty ≠ amount). Item AMOUNT now shows original gross (qty × unit price) in all three renderers — `api.ts` mapBill (`lineTotal = unit_price × qty`) covers the on-screen bill + HTML PDF + WhatsApp text; `pdfgen.py` items loop uses `qty × unit_price`. Discount stays a single line in the totals block. Tax math unchanged (still computed on the discounted taxable internally); line amounts now sum to Subtotal so the invoice reconciles cleanly.
  - PDF font visibility: darkened the barely-legible `#9AA0AC` labels/footer to `#6B7280` in both generators; "Powered by / Generated by Claro" now renders in brand plum `#2D1150`.
  - Two-column power wheel: signed powers (SPH/CYL) split into left Plus(+) / right Minus(−) columns (`signedSplitOptions`) so the optician scrolls half as far; active column highlighted in accent. AXIS/ADD keep the single wheel. Blank "—"/Clear still optional.
  - Hosted-PDF WhatsApp link diagnosed live: `share-link` returned `{url:null}`. Root cause #1: `storage_enabled()` needed SUPABASE_URL which wasn't set on Render — fixed `storage.py` to derive the Supabase URL from DATABASE_URL's project ref, so it works with ONLY `SUPABASE_SERVICE_ROLE_KEY` set. Root cause #2 (current): Supabase returns `403 "signature verification failed"` → the service-role key value on Render is wrong/mismatched for project dmcuwrtjtnigktqxccln. `share-link` now returns a `reason` (`no_service_key` / `no_supabase_url` / `bad_service_key` / `http_*`) for visibility without server logs.
- Verify: `tsc --noEmit` clean; URL-derive unit-checked for pooler + direct DATABASE_URL; PDF smoke-renders with a discounted bill; live `share-link` confirmed storage now ENABLED (reason surfaced the real upload error).
- Ship: backend pushed (Render auto-redeploy); frontend `eas update --branch preview` (group 2dbc8a52). Runtime 0.1.0 — existing installs update with no reinstall.
- OWNER ACTION (blocks the WhatsApp PDF link only): re-copy the **service_role** secret from Supabase → project dmcuwrtjtnigktqxccln → Settings → API → `service_role` (a long `eyJ…` JWT, NOT the JWT Secret, NOT the anon key), paste into Render env `SUPABASE_SERVICE_ROLE_KEY` with no surrounding spaces/quotes/truncation, save, redeploy. The "Share / Save" button already attaches the real PDF regardless.
- Open: confirm the PDF link works once the key is corrected; remove the temporary live test business on beta account +919876543210 if desired.
