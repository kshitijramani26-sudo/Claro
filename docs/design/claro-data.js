/* Claro — isolated mock data. Swap for a real API later with zero layout change. */
(function () {
  const shop = {
  ...
  // Recent activity feed (variant C)
  const activity = [
  ...
  // Tab 3 — Inventory
  const inventoryStats = { totalValue: 286400, skus: 142, lowCount: 3 };
  const inventory = [
  ...
  // Searchable inventory for Create Bill
  const billCatalog = inventory.map((i) => ({ id: i.id, name: i.name, price: i.price }));

  window.CLARO = {
    shop, summary, activity, khata, khataTimeline,
    inventoryStats, inventory, staff, staffDetail,
    analytics, bestSelling, billCatalog,
  };
})();
