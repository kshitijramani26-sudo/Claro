...
async def _assert_customer_visible(conn, biz: CurrentBusiness, customer_id: UUID) -> None:
    """Staff may only touch khata for customers they created (server-enforced)."""
    if not biz.is_staff:
...
async def khata_list(biz: CurrentBusiness = Depends(get_current_business)) -> KhataListRead:
    # Staff see only the khata of customers they created.
    staff_scope = "" if not biz.is_staff else " AND c.created_by_member_id = $2"
    args: list = [biz.id] + ([biz.member_id] if biz.is_staff else [])
    async with biz_txn(biz.id) as conn:
        rows = await conn.fetch(
            f"""SELECT c.id, c.name, COALESCE(c.phone, '') AS phone, c.outstanding_balance_paise,
                      COALESCE((SELECT max(k.created_at) FROM khata_entries k WHERE k.customer_id = c.id),
                               c.created_at) AS updated_at
               FROM customers c
               WHERE c.business_id = $1 AND c.outstanding_balance_paise > 0{staff_scope}
...
async def add_credit(payload: KhataCreditCreate, biz: CurrentBusiness = Depends(get_current_business)) -> KhataCustomerRead:
    async with biz_txn(biz.id) as conn:
        if payload.customer_id is not None:
            await _assert_customer_visible(conn, biz, payload.customer_id)
            ...
            if phone:
                cust_id = await conn.fetchval(
                    """INSERT INTO customers (business_id, name, phone, created_by_member_id) VALUES ($1, $2, $3, $4)
                       ON CONFLICT (business_id, phone) WHERE phone IS NOT NULL DO UPDATE SET name = EXCLUDED.name
                       RETURNING id""",
                    biz.id, name, phone, biz.member_id,
                )
            else:
                cust_id = await conn.fetchval(
                    "INSERT INTO customers (business_id, name, created_by_member_id) VALUES ($1, $2, $3) RETURNING id",
                    biz.id, name, biz.member_id,
                )
        await conn.execute(
            """INSERT INTO khata_entries (business_id, customer_id, type, amount_paise, note,
                                          created_by_member_id, performed_by_member_id)
               VALUES ($1, $2, 'credit', $3, $4, $5, $5)""",
            biz.id, cust_id, payload.amount_paise, payload.note or "Credit added", biz.member_id,
        )
...
async def settle(customer_id: UUID, payload: SettleCreate, biz: CurrentBusiness = Depends(get_current_business)) -> KhataCustomerRead:
    async with biz_txn(biz.id) as conn:
        ...
        await conn.execute(
            """INSERT INTO khata_entries (business_id, customer_id, type, amount_paise, note, performed_by_member_id)
               VALUES ($1, $2, 'payment', $3, $4, $5)""",
            biz.id, customer_id, amount, note, biz.member_id,
        )
        row = await conn.fetchrow(
            """UPDATE customers SET outstanding_balance_paise = outstanding_balance_paise - $3
               WHERE id = $1 AND business_id = $2
