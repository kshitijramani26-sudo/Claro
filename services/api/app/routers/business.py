...
def _read(row: dict, phone: str) -> BusinessRead:
    return BusinessRead(
        ...
        low_stock_default=row["low_stock_default"], email=row["email"], phone=phone,
    )
...
async def get_business(biz: CurrentBusiness = Depends(get_current_business)) -> BusinessRead:
    return _read(biz.row, biz.user.phone)
...
async def patch_business(payload: BusinessPatch, biz: CurrentBusiness = Depends(get_current_business)) -> BusinessRead:
    ...
    return _read(dict(row), biz.user.phone)
